...
public class CreateTodoListCommandUnitTests : AbstractTest
{
    [Fact]
    public async Task CreateTodoList_ShouldCreateNewTodoList()
    {
        var todoLists = TodoListFaker.Generate(10);

        var todoListRepoMock = new Mock<ITodoListRepository>();
        todoListRepoMock.Setup(
            x => x.Add(It.IsAny<Domain.Models.TodoList>(), It.IsAny<CancellationToken>())
        ).ReturnsAsync((Domain.Models.TodoList x, CancellationToken _) => x);
        ...
        var handler = new CreateTodoListCommandHandler(todoListRepoMock.Object, mediatorMock.Object);
        ...
        Assert.Equal("Todo list created.", res.Message);

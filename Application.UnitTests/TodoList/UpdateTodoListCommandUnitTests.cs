...
public class UpdateTodoListCommandUnitTests : AbstractTest
{
    [Fact]
    public async Task UpdateTodoList_ShouldUpdateTodoList()
    {
        ...
        todoListRepoMock.Setup(
            x => x.Update(
                It.IsAny<long>(),
                It.IsAny<Domain.Models.TodoList>(),
                It.IsAny<CancellationToken>()
            )
        ).ReturnsAsync((long _, Domain.Models.TodoList x, CancellationToken _) => x);
        ...
        var handler = new UpdateTodoListCommandHandler(todoListRepoMock.Object, mediatorMock.Object);
        ...
        Assert.Equal("Todo list updated.", res.Message);
    ...
    [Fact]
    public async Task UpdateTodoList_ShouldReturnNotFoundWhenTodoListNotExists()
    {
        var todoListRepoMock = new Mock<ITodoListRepository>();
        todoListRepoMock.Setup(
            x => x.Exists(It.IsAny<long>(), It.IsAny<CancellationToken>())
